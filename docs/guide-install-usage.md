...
CometFlow 是一个**全时运行的自主 Agent 开发平台**。一句话讲清分工：
...
准备三个 tarball：`zqg-cometflow-0.1.0.tgz`、`commander-14.0.3.tgz`、`yaml-2.9.0.tgz`。

```bash
mkdir cometflow-install && cd cometflow-install
npm init -y
npm install --offline ./zqg-cometflow-0.1.0.tgz ./commander-14.0.3.tgz ./yaml-2.9.0.tgz
