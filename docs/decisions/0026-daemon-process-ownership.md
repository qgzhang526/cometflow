...
## 修订（2026-09-17）：进程主人从「CLI」扩到「托管它的进程」

[ADR 0027](./0027-embedded-scheduler.md) 落地了 serve 内嵌调度器，本文结论中的"进程归 CLI"随之放宽为：

> **进程归托管它的那个进程**——CLI（`daemon start`）或 serve（页面上的「启动调度器」，以 job 形式）。
> 两条路调用同一个循环、共用同一把单实例租约；**暂停 / 继续 / 停止仍然只写控制文件**，
> 本文关于"控制是状态而不是信号、界面不必拥有进程"的论证完全不变。

也就是说：本文要防的从来不是"serve 能不能托管调度器"，而是"浏览器 spawn 一个游离进程、日志与退出无处安放"。
内嵌调度器以 job 形式跑，日志进任务中心、停止走控制文件，恰好满足本文的约束。
