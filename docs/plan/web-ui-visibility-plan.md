...
状态：**V1–V4 全部完成**（见 §3.1–§3.4，均含测试与浏览器走查）；审计的产品缺口清零
...
| ✅ V4-2 | TopBar 健康徽章 | 审计 C14 | V1-1 的 findings（提升为共享 store） | S | V1-1 |
| ✅ V4-3 | 目标单条编辑/删除 | 审计 C16 | `mission.md` 分段写 + `goal-sync` | M | 无 |
| ✅ V4-4 | Classic 定位 → **界面移除 + 标注弃用**（用户决策：只保留 native） | 审计 C10 | — | S | 需先定方向（已定） |
| ✅ V4-5 | B 类两个端点：决策为**标注保留**，不删除 | 审计 §6 | — | S | 无 |
| ✅ V4-6 | 调度预算可见（`daemon budget` 的只读投影） | 审计 §5.1 | `scheduler/budget.ts` | S | 无 |
...
## 3.4 实施结果（V4 已完成，2026-09-16）

| 项 | 落点 | 验证 |
|---|---|---|
| V4-1 Eval 历史对比 | `EvalPanel.vue`：按 `kind=eval-run` 列出带报告的历史（任务中心已持久化，无需新端点），可选一轮作「对比轮」，给出 Pass@k / Pass^k / 通过轮次的差值与「结论翻转」的任务清单 | 浏览器实测：跑两轮 eval → 历史表出现 2 条 → 设为对比轮 → 差异表与「没有任何任务改变结论」 |
| V4-2 TopBar 健康徽章 | 新增 `stores/findings.ts`（findings 是唯一被两处消费的投影：顶栏徽章 + 总览清单）；`ProjectView` 打开项目时加载，顶栏显示 `N error / M warning` 并可点回总览 | 浏览器实测：顶栏出现「1 error / 1 warning」，与总览清单同源同数 |
| V4-3 目标单条编辑/删除 | `GoalsPanel.vue`：按 `### Gn` 块边界就地替换/删除，写回 `COMETFLOW.md` 后自动 `goal sync`；删除前弹窗显示将被删除的原始块 | 浏览器实测：改 G2 标题 → 四个块都还在、仅 G2 变化（629 → 637 字节）；删 G3 → 确认框显示原块、删除后 G1/G2/G4 完好 |
| V4-4 Classic 界面移除 | 资产面板去掉 Classic 页签（现为 Skills / Bundle / Hook 预览）；USAGE §7.4 标注弃用与后续移除计划 | 浏览器实测页签集合；CLI 与 `GET /classic` 暂留以便存量项目过渡 |
| V4-5 冗余端点 | **决策改为「标注保留」**：`/config/project` 与 `/spec-index` 都有 USAGE 文档与既有测试（`serve-api.test.ts`），删除它们是移除对外表面而不是清理噪音；现在两处代码注释写明「保留给 CLI/脚本，界面不消费」，并补测试钉住 | 新测试断言两者仍可用；删掉我最初写的「应返回 404」用例 |
| V4-6 调度预算 | `GET /scheduler/queue` 增加 `budget`（跨重启累计的 `used_ms` + `updated_at`，来自 `readBudgetUsage`），调度面板显示「已用预算」并注明重置走 CLI | 新测试断言字段存在；浏览器实测「已用预算 0 ms」 |

V4-5 的过程值得记一笔：我最初按审计建议**删掉**这两个端点，结果 `serve-api.test.ts` 立刻红了两处——我先前用来判断「没有测试依赖」的 grep 输出被 `Select-Object -First 15` 截断，结论是错的。测试把错误的判断挡了下来，随后改为「标注保留」。**审计给的是两个选项，选哪个要以「是否移除对外表面」为准，不能只按「界面没用到」判断。**

测试与验证：全量 `npx vitest run` → **77 文件 / 463 例全绿**；回归 **110 步 PASS**；两个 typecheck 通过；浏览器走查覆盖六项。
...
| V4 余项 ✅ | V4-1 ~ V4-6 | 审计 §1 的「产品缺口」清零：剩下的无 UI 入口全是安装/运维/长驻进程类，或已按决策保留/弃用（见 §3.4） |
