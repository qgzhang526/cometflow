...
| 里程碑 | 内容 | 完成标志 |
|---|---|---|
| V1 可见性清零 | V1-1 ~ V1-4 | 总览一屏能回答「现在有哪些问题、质量指标如何、谁在阻塞写入、要不要清理」；审计里 C1/C2/C3 三条从 ✗/△ 变 ✓ |
...
| # | 问题 | 选项 | 建议 |
|---|---|---|---|
| 1 | 是否把「统一 findings」纳入 V1？（它不在审计 §9 的原始 9 条里，是 P5 落地后新出现的投影） | A 纳入 V1-1；B 放 V3 | **A**：它是 C1/C14 的天然数据源，先做它等于少写一套 doctor 专用渲染 |
| 2 | doctor 维护动作的写语义 | A 界面直接执行（带 dry-run 数字 + 二次确认）；B 界面只显示命令、让用户回 CLI | **A**，但 `force-unlock` 必须显示持有者并要求显式确认 |
| 3 | metrics 卡片是否显示门禁阈值 | A 显示；B 只显示指标 | **A**：P3 的动机就是「看不见的约束等于没有约束」，界面上同样成立 |
| 4 | `expected` 数字不匹配时的行为 | A 409 拒绝；B 警告后照删 | **A**：删除不可逆，宁可让人重看一眼 |
