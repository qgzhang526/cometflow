...
| 维度 | 调研时 | W1–W5 完成后 | 说明 |
|---|---|---|---|
| CLI 叶子命令 | 65 | 65+ | 22 个命令组（后续由加固计划继续增加） |
| HTTP 端点 | 49 | ≈70 | 新增 spec 内核 9、change 审计 5、job 清理 1、资产与门禁 6 |
| 前端面板 | 8 | 10 | 新增「调度」与「资产」（资产内含 Skills / Bundle / Classic / Hook 预览） |

结论：**spec 内核与 change 审计这两块「产品主张」原本在前端几乎不可见**。
`spec` 12 条子命令里前端只用了 `validate` / `scaffold` / `index`；`change` 11 条子命令里只用了
`new` / `transition` / `run` / `verify` / `archive`。也就是说，CLI 里最贵的那部分能力
（版本回放、影响分析、漂移检测、一致性门禁、实现范围审计）当时只有敲命令才看得到——W2/W3 已经把这些补上。
...
### W4 事件与 job 体系收口（B 类 + P1-5/6）✅ 已完成
...
实施结果：SSE 部分（按 `projectId`/`path` 路由、去抖、编辑中挂起、退避重连并提示、`job.*` 事件驱动日志、
面板不再轮询、TopBar 任务指示器）在 W1 修 P0 时已落地；W4 只做收口与验收：

- **保留上限**：`JobManager` 只保留最近 200 个任务，超限时从**已结束**的任务里淘汰最旧的，
  排队/运行中的永不淘汰（淘汰它们等于丢证据）。
- **手动清理**：新增 `DELETE /api/jobs`（只删已结束）+ 任务中心「清理已完成」按钮，
  长跑的 serve 不会让任务中心越用越慢。
- **面板与任务联动**：面板不再只认「本次会话启动的 jobId」，而是按 `kind + change` 从任务列表里
  找回最近一次运行——刷新页面、换会话之后，change 的运行日志仍在。
- **深链**：任务中心可跳到对应 change（`#/project/<id>/changes?change=<name>`），变更面板据此自动选中。
