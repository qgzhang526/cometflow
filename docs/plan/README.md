...
| 平台侧后续 | 独立验证默认化、hook 真正接线、调度器健壮性（并发写锁的设计归 web-ui-followup-plan 的 N4） | 待开始 | [platform-next-plan.md](./platform-next-plan.md) |
| Web 前端后续 | 引用图、引用高亮、Job 持久化、并发写保护、编辑语义、收尾 | 计划，待评审 | [web-ui-followup-plan.md](./web-ui-followup-plan.md) |
...
| Web 前端后续 | 引用关系图与引用高亮、Job 持久化、并发写保护、spec 编辑语义、发布链路收尾 | 计划，待评审 | [web-ui-followup-plan.md](./web-ui-followup-plan.md) |
