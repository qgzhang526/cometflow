...
| P4 Workflow 深度融合 | 让 daemon 驱动 change 生命周期（取任务 → 建 change → 执行 → 独立验收 → 归档 → 回写交付事实），并把队列降级为派生视图、决定 daemon 进程生命周期归属 | **计划（未实施）**（2026-09-17，分支 `codex/daemon-drives-change`） | [daemon-drives-change-plan.md](./daemon-drives-change-plan.md) |
