...
|---|---|---|---|---|
| 度量投影 | `metrics` | `metrics-service.ts` / `spec-health.ts` | 无端点、无面板 | 重建质量（first_pass / pass / blocked / check_coverage）与 spec 健康度（acceptance_checkable、anchor_coverage、drift、versions）现在只有 CLI 与 CI 基线能看到；**这是当前最大的一块「后端最贵、前端零可见」** |
| 当前 change 指针 | `change select` | `current-change.ts` | 无端点、无入口 | 多个活跃 change 时 hook 守卫 **fail closed**（ADR 0018）；界面只能看到「写入被拒」，却无法在界面里指定指针解除，必须回 CLI |
...
| 编号 | 问题 | 证据 | 影响 |
|---|---|---|---|
| C1 | doctor 只在总览里以纯文本列出，且维护动作无入口 | `OverviewPanel.vue` 的 Doctor 卡；`doctor.ts` 的三个开关 `cleanTemp/cleanJobs/forceUnlock` 只接受 CLI 参数，findings 文案直接写「运行 `cometflow doctor . --clean-temp`」 | 用户读到了「该清理了」，但只能切回终端；008 §S5 要求的「诊断面板」没有独立落地 |
| C2 | 度量投影零可见 | `collectMetrics` 有 `rebuild` + `spec_health` 两组指标，`web/src` 无任何调用 | 「重建质量有没有变好」在界面上无法回答；CI 基线与开发者看到的不是同一组数字 |
| C3 | current-change 指针无入口 | `change select` 只在 CLI；`AssetsPanel` 的 Hook 预览文案写着「多个活跃 change 且没有 current-change 指针时 fail closed」，界面却无法设置指针 | 多活跃 change 的项目里，用户在界面上只会看到「写入被拒」，恢复路径在终端 |
...
#    因此 method+path 组合为 76 条，再加 serve.ts 的 /api/events = 77）
