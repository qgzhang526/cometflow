...
| 调度顺序（goal 级） | `goal sync`（回显）、`daemon queue rebuild`（打印） | `GET /scheduler/queue` 的 `order`（`listed`/`warnings`） | 调度（「调度顺序」卡 + 队列行的 `#N`） | 在 COMETFLOW.md 写 `## 调度顺序` 列出 `- G3` → `goal sync .` 与 `daemon queue rebuild .` 打出同一份顺序；不写清单时按 goal 编号升序（`G2` 在 `G10` 前） |
