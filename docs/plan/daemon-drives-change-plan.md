...
## 9. 并发与精细化排序：现状与前置条件（未实施）

本批按 §4 第 3 条刻意做成**单任务串行**（按计划文件顺序取第一个 `queued`，无优先级、无并行）。
这不是"方案已就绪但没排上"，而是**方案没做**——要做之前必须先解决四件事，否则并发会把现有保证打穿：

| # | 前置条件 | 现状问题 |
|---|---|---|
| 1 | **任务领取的原子性** | `markQueueTask` + `writeQueue` 是读改写，领取不是原子操作。两个 daemon 实例（CLI 一个、serve 内嵌一个、或两台机器）可能**同时选中同一个任务**。要先有 CAS 或项目级锁（`platform/fs/file-lock.ts` 可复用） |
| 2 | **change 级互斥** | 只有 `archiveChange` 拿锁，`runChange` / `verifyChange` 没有。两条通道同时驱动同一个 change 会真跑两次 agent（P4 让「别人在飞」的任务被跳过，但**同一 change 被同时驱动**没有硬保护） |
| 3 | **指针与写保护的配合** | 多 change 并发时 `current-change` 指针只能指一个，ADR 0018 的 fail closed 会让 agent 的写入被拒。要么按 `module` 边界分区（spec 已声明 module，可直接当并发单元），要么按 goal 串行 |
| 4 | **排序语义** | 目前顺序 = 计划文件顺序。优先级该来自哪里（goal 顺序 / 任务 `depends_on` 依赖图 / 预算成本）？抢占与跳过怎么定义？依赖图**已经存在于 plan 里，但调度器完全没用它** |

建议的实施顺序（每一步都能单独验收）：

1. 先做 #1 + #2（原子领取 + change 级锁）——它们不改变功能，只是把「already 正确」的东西变成「并发下也正确」；
2. 再做 #4 的最小版：按 `depends_on` 拓扑排序 + goal 内串行（不引入优先级配置）；
3. 最后做 #3 的并发上限（`--concurrency N`，并发单元 = module 或 goal），并同时把预算与指针策略讲清楚。

顺带记一个已知粗糙点：**「需人工介入」停机之后，把单条任务重新排队没有命令**——
现在只能 `daemon queue reset`（会一并清掉其它任务的覆盖与 attempts 计数）。
建议补 `daemon queue retry <goal:task>`（单条重排、保留其余覆盖），与 #1 的原子领取一起做。
