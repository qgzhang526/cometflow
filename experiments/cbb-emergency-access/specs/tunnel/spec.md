...
`src/tunnel/forwarder.mjs`，由 配置：tunnel.forwarder_module 指定（相对路径按进程 cwd 解析）。
...
验收用 `tests/fixtures/fake-forwarder.mjs` 替换，异常路径用 `tests/fixtures/failing-forwarder.mjs`。
...
- A12：回收后会话状态变为 ended，end_reason 被记录，且原有转发规则不再包含临时规则
  - check: node tests/acceptance.mjs A12
- A13：对已回收会话重复调用返回 200，且不产生额外变更
  - check: node tests/acceptance.mjs A13
