...
- A4：approver 审批通过后返回 200，申请单状态变为 approved，并生成一次性授权
  - check: go test ./tests/acceptance -run '^TestA4$' -count=1
- A5：审批人等于申请人时返回 403 与 E_SELF_APPROVAL
  - check: go test ./tests/acceptance -run '^TestA5$' -count=1
- A6：对已审批的申请单再次审批时返回 409 与 E_REQUEST_ALREADY_DECIDED
  - check: go test ./tests/acceptance -run '^TestA6$' -count=1
...
- A8：查询既有申请单返回 200，包含申请、授权与会话状态，且响应中不出现明文令牌
  - check: go test ./tests/acceptance -run '^TestA8$' -count=1
