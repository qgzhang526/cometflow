...
| 形态 | 契约 |
|------|------|
| 进程 | `go run ./cmd/server --config <config.json> --port <port>`（构建产物为 `go build -o server ./cmd/server`）；监听就绪后在 stdout 打印 `listening on <port>`（`--port 0` 表示由系统分配端口）；收到 SIGTERM/SIGINT 时优雅退出码 0 |
| 模块 | `internal/app` 的 `Start(app.Options) (app.Server, error)` → `app.Server` 提供 `URL() string` 与 `Close() error`；`Options.ConfigPath` 为配置文件绝对路径 |

配置文件为 JSON，键按 config.md 的点号分层（`access.max_duration_minutes` →
`{"access":{"max_duration_minutes":30}}`）；配置里的相对路径按进程 cwd 解析。

验收执行器（`tests/acceptance`）用模块形态在同一进程内起服务，因此模块形态是**判据的一部分**，
不允许只有进程形态。外部依赖（转发器、时钟）通过 `app.Options` 注入，见 config.md「接缝」。
